package com.longer.aop_login.aspect;


import android.util.Log;

import com.longer.aop_login.annotation.ClickBehavior;
...
public class ClickBehaviorAspect {
    private final static String TAG = "longer >>> ";

    // 1、应用中用到了哪些注解，放到当前的切入点进行处理（找到需要处理的切入点）
    // execution，以方法执行时作为切点，触发Aspect类
    // * *(..)) 可以处理ClickBehavior这个类所有的方法
    @Pointcut("execution(@com.longer.aop_login.annotation.ClickBehavior * *(..))")
    ...
    public Object jointPoin(ProceedingJoinPoint joinPoint) throws Throwable {
        Log.e(TAG, "进来了");
        // 获取签名方法
